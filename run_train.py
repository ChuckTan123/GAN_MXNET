...
def train(mods, rand_iter, image_iter, batch_size, ctx, epochs):
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(image_iter):
            ...
            # lossDis
            out_dis3 = discriminator.get_outputs()[0]
            acc3 = cal_eval(out_dis3, label)
            
            print ("Generator Acc is {} (the lower the better), the Discriminator is {} (the higher the better)".format((acc1+acc2)/2, acc3))
...
def run_train():
    g = get_generator()
    d = get_discriminator()
    rand_iter, image_iter = getData("data/AnmineFaceSmall.rec", (3,64,64), BATCH_SIZE)
    mods = getMods(g, d, rand_iter, image_iter, CTX, SIGMA, LR, BETA1)
    train(mods, rand_iter, image_iter, BATCH_SIZE, CTX, 100)
